            ...
            {rules.map((r) => (
              ...
              >
                ...
                <div className="space-y-1.5 text-xs mb-4">
                  ...
                  <Row label="Minimum Limit" value={`₺${r.limit}`} />
                  <Row label="Min. Marj" value={`%${r.minMargin}`} />
                </div>

                <button
                  onClick={() => toggle(r)}
                  className={`mt-auto inline-flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold transition-all ${
                    r.isActive
                ...
                >
                  ...
                  {r.isActive ? 'Aktif (Devre Dışı Bırak)' : 'Pasif (Etkinleştir)'}
                </button>
              </div>
            ))}
          </div>
